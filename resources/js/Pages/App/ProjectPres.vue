  ...
  <PresentationLayout>
    <Box class="space-y-2 bg-primary/80 relative" v-if="projectFound">
      ...
      <template v-for="note in project.notes" class="" :key="note.id">
        ...
        <div class="flex flex-row m-4 rounded border p-4 shadow-2xl bg-neutral/70">
          <div>
            ...
            <svg v-if="note.note_type_code === 'monetization'" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                 viewBox="0 0 24 24" fill="none"
                 stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
          ...
          <pre class="text-wrap font-sans ml-4">{{ note.content }}</pre>
        </div>
      </template>
